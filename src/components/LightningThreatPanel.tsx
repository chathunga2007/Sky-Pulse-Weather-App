...
import type { WeatherCurrent, WeatherHourly } from "../types/weather";

interface LightningThreatPanelProps {
  current: WeatherCurrent | null;
  hourly: WeatherHourly | null | undefined;
  currentCity: { name: string; country?: string } | null;
}

interface CalculatedDistance {
  km: string;
  miles: string;
  seconds: string;
  danger: boolean;
...
export default function LightningThreatPanel({
  ...
  const getThreatData = (capeVal: number, code: number) => {
  ...
  const [calculatedDistance, setCalculatedDistance] = useState<CalculatedDistance | null>(null);
